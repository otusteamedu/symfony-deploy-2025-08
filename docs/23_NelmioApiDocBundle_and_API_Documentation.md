...
3. В файле `config/packages/nelmio_api_doc.yaml` в секцию `documentation` добавляем параметр
   ```yaml
   nelmio_api_doc:
      documentation:
          openapi: 3.1.0
   ```
4. Заходим по адресу `http://localhost:7777/api/doc.json`, видим JSON-описание нашего API
5. Заходим по адресу `http://localhost:7777/api/doc`, видим ошибку
...
5. В консоли выполняем
   ```shell
   php bin/console nelmio:apidoc:dump --format yaml --area=feed
   ```
   видим сгенерированный ответ
