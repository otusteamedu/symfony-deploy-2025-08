...
В версии ApiPlatform 4.1 (возможно и дальше), возможна ошибка с отсутствующим `/app/public/manifest.json`. 
В таком случае нужно установить, например, `yarn` и собрать "фронтовую" часть. Шаги аналогичны уроку [Twig и Symfony Forms](10_Twig_and_Symfony_Forms.md)
1. Устанавливаем `yarn` в контейнере (либо добавляем в образ, если планируем использовать дальше) командой
   `apk add yarn`
2. Устанавливаем зависимости командой `yarn install`
3. Собираем командой `yarn build`
...
**Напоминание**: чтобы всё работало верно, нам нужны открытый и закрытые ключи для создания токенов. 
Создание ключей описано в уроке [Stateless API](12_Stateless_API.md).
```shell
    mkdir config/jwt
