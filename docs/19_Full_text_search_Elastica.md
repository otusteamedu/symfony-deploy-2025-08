...
2. В файле `.env` исправляем DSN для ElasticSearch
    ```shell script
    ELASTICSEARCH_DSN=https://%env(ELASTICSEARCH_USER)%:%env(ELASTICSEARCH_PASSWORD)%@elasticsearch:9200
    ```
...
5. В файле `config/packages/fos_elastica.yaml` в секции `clients` меняем `url` на `hosts`
```yaml
    clients:
        default:
...
```
в секции `fos_elastica.indexes` удаляем `app` и добавляем секцию `user`:
```yaml
user:
    persistence:
        driver: orm
    ...
    properties:
        ...
        email: ~
```
6. Поправляем `src/Domain/Entity/EmailUser.php`
```php
...
```
7. Заполняем индекс командой `php bin/console fos:elastica:populate`
8. Заходим в Kibana по адресу `http://localhost:5601`
9. В Kibana заходим в Discover. В левом верхнем углу разворачиваем `Data view` В правом нижнем углу менюшки `Create a data view` и создаём index pattern на базе индекса `user`
10. Видим наши данные в новом шаблоне, причём телефон и email корректно заполняются
...
2. Добавим индекс с составными полями в `config/packages/fos_elastica.yaml` в секцию `fos_elastica.indexes`
    ...
    tweet:
        ...
        properties:
            ...
            text: ~
    ```
3. В контейнере ещё раз заполняем индекс командой `php bin/console fos:elastica:populate --index=tweet`
4. В Kibana заходим в `Discover` в левом верхнем углу разворачиваем `Data view` В правом нижнем углу менюшки `Create a data view`  и создаём index pattern на базе индекса `tweet`
...
1. В файле `config/packages/fos_elastica.yaml`
    1. Включаем сериализацию
        ...
        serializer: ~
        ```
    2. Для каждого индекса (`user`, `tweet`) удаляем секцию `properties` и добавляем секцию `serializer` на уровне `persistence`
        ```yaml
        serializer:
            groups: [elastica]
        ```
