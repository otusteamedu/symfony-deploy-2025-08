...
## Избавляемся от WorldController'а

1. Удаляем из файла `config/routes.yaml` секцию с `hello_world`
   ```yaml
   hello_world:
       path: /world/hello
       controller: App\Controller\WorldController::hello
   ```
2. Удаляем контроллер `App\Controller\WorldController`
...
3. Исправляем класс `App\Controller\Web\UpdateUserLogin\v1\Controller`
    ...
    ```
4. Выполняем запрос Patch user by id из Postman-коллекции с несуществующим id, видим ошибку `404`
