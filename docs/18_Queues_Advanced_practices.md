...
3. Добавляем класс `App\Infrastructure\Repository\SmsNotificationRepository`
    ...
    ```
4. Добавляем класс `App\Domain\Service\SmsNotificationService`
    ```php
    <?php
...
6. Добавляем интерфейс `App\Domain\Bus\UpdateFeedBusInterface`
    ...
    ```
7. Исправляем перечисление `App\Infrastructure\Bus\AmqpExchangeEnum`
    ```php
    <?php
    ...
    ```
8. Добавляем класс `App\Infrastructure\Bus\Adapter\UpdateFeedRabbitMqBus`
    ```php
    <?php
    ...
    ```
9. Исправляем класс `App\Controller\Amqp\PublishTweet\Consumer`
   ```php
   <?php
   ...
   ```
10. Добавляем описание нового продюсера и консьюмеров в файл `config/packages/old_sound_rabbit_mq.yaml`
     1. в секцию `producers`
         ```yaml
         update_feed:
             connection: default
     ...
     2. в секцию `consumers`
          ...
          update_feed_9:
              ...
              qos_options: {prefetch_size: 0, prefetch_count: 1, global: false}
          ```
11. В файл `config/services.yaml` добавляем к сервису `App\Infrastructure\Bus\RabbitMqBus` регистрацию нового продюсера:
     ```yaml
