...
1. В классе `App\Domain\Entity\Subscription` добавляем атрибут `ORM\HasLifecycleCallbacks` для класса и атрибуты для методов
...
5. Добавляем класс `App\Controller\Amqp\AddFollowers\Consumer`
    ...
    ```
6. Добавляем перечисление `App\Infrastructure\Bus\AmqpExchangeEnum`
    ```php
    <?php
...
12. Исправляем класс `App\Domain\Service\FollowerService`
    ...
    ```
13. Исправляем класс `App\Controller\Web\AddFollowers\v1\Manager`
    ```php
    <?php
    ...
    ```
14. В файл `config/services.yaml` добавляем новый сервис
     ```yaml
     ...
     ```
15. Запускаем консьюмер командой `php bin/console rabbitmq:consumer add_followers -m 100`
16. Выполняем запрос Add followers из Postman-коллекции v8 с параметром `async` = 1, видим в интерфейсе RabbitMQ
