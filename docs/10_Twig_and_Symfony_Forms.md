...
4. Устанавливаем загрузчик для работы с SASS командой `yarn add sass sass-loader@^14.0.0 --dev` (или можно без указания версии, тогда установится самая новая `yarn add sass sass-loader --dev`)
...
7. Исправляем файл `assets/app.js`
    ...
    require('bootstrap');
    ```
8. Исправляем файл `webpack.config.js`, убираем комментарий в строке 60 (`//.enableSassLoader()`)
